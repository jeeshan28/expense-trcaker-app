...
router.post('/forgot',async function(req, res, next) {
  ...
  try {
    ...
    if(!userF){
      checkUser = 'username did not found'
      res.redirect('/forgot')
    } else {
  ...
  try {
    ...
    if(!userF1){
      ...
      if(userF1.genratedOtp === +req.body.genratedOtp){
        ...
        userF1.save();
        res.render('newPassword',{loginUser:req.user,userMail:userF1.email,checkUser:checkUser});
      } else{
...
async function sendmailhandler(email, otp, res,checkUser){
  ...
  const transport = nodemailer.createTransport({
    ...
    port:465,
    auth:{
      user: "kjeeshan312@gmail.com",
      pass: "qidl lmmp jugq yuvx",
    },
    // tls:{
  ...
  // receiver mail info 
  const mailOptions = {
    from:'Jeeshan Pvt. Ltd.<kjeeshan312@gmail.com>',
    to:email,
...
// post route to update post details
router.post('/post-update/:id',isLoggedIn, async (req, res)=>{
  try {
   ...
   console.log(error);
   res.send('error aya hai ',error) 
  }
})

// middleware to check is user is login or not
